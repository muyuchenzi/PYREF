...
def multi_thread_queue_prase(html_queue, file_save):
    while True:
        ...
        print(threading.current_thread().name, f"parse:{len(results)}", f"html_queue.size:", html_queue.qsize())
        # print(results)
...
def run():
    ...
    for i in range(3):
        ...
        t.start()
    # parse_threads = []
    file_save = open(r"E:\李震祥\PYGIT\PYref\ReviewCode\QA_for_InterView\Multi_process_thread\craw_data.txt", "w")
    for i in range(5):
